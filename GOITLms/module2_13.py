...
# Последние три буквы алфавита зацикливаются и переносятся в начало. Буква X становится A, Y – B, а Z – C.
...
for ch in message:
    if ch.isalpha():
        # Определение смещения относительно a (для строчных и прописных букв)
        base_ord = ord('a') if ch.islower() else ord('A')
        pos = (ord(ch) - base_ord + offset) % 26
        new_char = chr(pos + base_ord)
